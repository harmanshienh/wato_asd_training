...
void MapMemoryNode::processParameters() {
  ...
  last_robot_x_ = INFINITY;
  last_robot_y_ = INFINITY;
}
...
void MapMemoryNode::localCostmapCallback(
  ...
  if (last_robot_x_ != INFINITY && last_robot_y_ != INFINITY)
  {
    //Euclidean distance
    double dist = sqrt(pow(robot_x_ - last_robot_x_, 2) + pow(robot_y_ - last_robot_y_, 2));
  ...
  // Update last position if we are far enough
  last_robot_x_ = robot_x_;
  last_robot_y_ = robot_y_;

  // Update the global map
  map_memory_.updateMap(msg, robot_x_, robot_y_, robot_angle_);
}

void MapMemoryNode::odomCallback(const nav_msgs::msg::Odometry::SharedPtr msg) {
  //Extract the robot’s position and orientation (Only need x, y since its 2d) from the Odometry message.
  ...
  robot_angle_ = quaternionToYaw(qx, qy, qz, qw);
}
